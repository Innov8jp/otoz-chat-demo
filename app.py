...
class SalesAgent:
    ...
    def _parse_intent(self, user_input):
        ...
        # FIX: More sophisticated parsing that maintains context
        # Extract entities from the current input
        all_makes = list(self.ss.inventory_df['make'].unique())
        make_match = re.search(r'\b(' + '|'.join(re.escape(m.lower()) for m in all_known_makes) + r')\b', text)
        make = make_match.group(1).title() if make_match else self.ss.query_context.get('make')
        ...
        if make or model or year or color:
            # Update the query context with new information
    ...
    def _handle_search_vehicle(self, params):
        ...
        # If the query is vague, ask for more details
